...
const CardForm = ({
  ...
  return (
    ...
    >
      {loading && (
        <div
          className={`absolute top-0 right-0 bottom-0 left-0 flex items-center justify-center bg-greyOpaque2 z-10 backdrop-blur-sm`}
        >
          <img
            src={spinner}
            className="h-[75px] w-[75px] relative bottom-[5%]"
          />
        </div>
      )}
      <div>
        ...
        {subtitle && (
          <p className="text-sm text-main3 w-full text-center font-medium">{subtitle}</p>
        )}
      </div>
      {!allowAutofill && (
        <>
      ...
      {link && (
        <p className="text-xs my-[5px] w-full text-center font-500">
          {link.text}{" "}
          <Link
            className="text-main1 hover:text-main1 hover:underline"
            to={link.route}
          >
            {link.linkText}
          </Link>
